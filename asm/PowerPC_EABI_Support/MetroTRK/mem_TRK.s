.include "macros.inc"
.file "PowerPC_EABI_Support/MetroTRK/mem_TRK.o"

# 0x802CBB7C - 0x802CBDE0
.text
.balign 4

.fn TRK_memcpy, global
/* 802CBB7C 0029513C  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 802CBB80 00295140  2C 05 00 00 */	cmpwi r5, 0x0
/* 802CBB84 00295144  39 20 00 FF */	li r9, 0xff
/* 802CBB88 00295148  93 E1 00 0C */	stw r31, 0xc(r1)
/* 802CBB8C 0029514C  93 C1 00 08 */	stw r30, 0x8(r1)
/* 802CBB90 00295150  7C 7E 1B 78 */	mr r30, r3
/* 802CBB94 00295154  41 82 01 14 */	beq .L_802CBCA8
/* 802CBB98 00295158  54 A0 F8 7F */	srwi. r0, r5, 1
/* 802CBB9C 0029515C  7C 09 03 A6 */	mtctr r0
/* 802CBBA0 00295160  41 82 00 B0 */	beq .L_802CBC50
.L_802CBBA4:
/* 802CBBA4 00295164  57 CC 00 3A */	clrrwi r12, r30, 2
/* 802CBBA8 00295168  54 8B 00 3A */	clrrwi r11, r4, 2
/* 802CBBAC 0029516C  7C 0C F0 50 */	subf r0, r12, r30
/* 802CBBB0 00295170  80 EB 00 00 */	lwz r7, 0x0(r11)
/* 802CBBB4 00295174  21 40 00 03 */	subfic r10, r0, 0x3
/* 802CBBB8 00295178  7C CB 20 50 */	subf r6, r11, r4
/* 802CBBBC 0029517C  20 06 00 03 */	subfic r0, r6, 0x3
/* 802CBBC0 00295180  38 84 00 01 */	addi r4, r4, 0x1
/* 802CBBC4 00295184  54 00 18 38 */	slwi r0, r0, 3
/* 802CBBC8 00295188  55 5F 18 38 */	slwi r31, r10, 3
/* 802CBBCC 0029518C  7C E0 04 30 */	srw r0, r7, r0
/* 802CBBD0 00295190  54 8B 00 3A */	clrrwi r11, r4, 2
/* 802CBBD4 00295194  54 00 06 3E */	clrlwi r0, r0, 24
/* 802CBBD8 00295198  81 0C 00 00 */	lwz r8, 0x0(r12)
/* 802CBBDC 0029519C  7D 2A F8 30 */	slw r10, r9, r31
/* 802CBBE0 002951A0  7C CB 20 50 */	subf r6, r11, r4
/* 802CBBE4 002951A4  7C 00 F8 30 */	slw r0, r0, r31
/* 802CBBE8 002951A8  3B DE 00 01 */	addi r30, r30, 0x1
/* 802CBBEC 002951AC  7D 07 50 78 */	andc r7, r8, r10
/* 802CBBF0 002951B0  38 84 00 01 */	addi r4, r4, 0x1
/* 802CBBF4 002951B4  7D 40 00 38 */	and r0, r10, r0
/* 802CBBF8 002951B8  7C E0 03 78 */	or r0, r7, r0
/* 802CBBFC 002951BC  90 0C 00 00 */	stw r0, 0x0(r12)
/* 802CBC00 002951C0  57 CC 00 3A */	clrrwi r12, r30, 2
/* 802CBC04 002951C4  7C 0C F0 50 */	subf r0, r12, r30
/* 802CBC08 002951C8  80 EB 00 00 */	lwz r7, 0x0(r11)
/* 802CBC0C 002951CC  21 40 00 03 */	subfic r10, r0, 0x3
/* 802CBC10 002951D0  81 0C 00 00 */	lwz r8, 0x0(r12)
/* 802CBC14 002951D4  20 06 00 03 */	subfic r0, r6, 0x3
/* 802CBC18 002951D8  3B DE 00 01 */	addi r30, r30, 0x1
/* 802CBC1C 002951DC  54 00 18 38 */	slwi r0, r0, 3
/* 802CBC20 002951E0  55 5F 18 38 */	slwi r31, r10, 3
/* 802CBC24 002951E4  7C E0 04 30 */	srw r0, r7, r0
/* 802CBC28 002951E8  54 00 06 3E */	clrlwi r0, r0, 24
/* 802CBC2C 002951EC  7D 2A F8 30 */	slw r10, r9, r31
/* 802CBC30 002951F0  7C 00 F8 30 */	slw r0, r0, r31
/* 802CBC34 002951F4  7D 07 50 78 */	andc r7, r8, r10
/* 802CBC38 002951F8  7D 40 00 38 */	and r0, r10, r0
/* 802CBC3C 002951FC  7C E0 03 78 */	or r0, r7, r0
/* 802CBC40 00295200  90 0C 00 00 */	stw r0, 0x0(r12)
/* 802CBC44 00295204  42 00 FF 60 */	bdnz .L_802CBBA4
/* 802CBC48 00295208  70 A5 00 01 */	andi. r5, r5, 0x1
/* 802CBC4C 0029520C  41 82 00 5C */	beq .L_802CBCA8
.L_802CBC50:
/* 802CBC50 00295210  7C A9 03 A6 */	mtctr r5
.L_802CBC54:
/* 802CBC54 00295214  57 CC 00 3A */	clrrwi r12, r30, 2
/* 802CBC58 00295218  54 8B 00 3A */	clrrwi r11, r4, 2
/* 802CBC5C 0029521C  7C 0C F0 50 */	subf r0, r12, r30
/* 802CBC60 00295220  80 EB 00 00 */	lwz r7, 0x0(r11)
/* 802CBC64 00295224  21 40 00 03 */	subfic r10, r0, 0x3
/* 802CBC68 00295228  7C CB 20 50 */	subf r6, r11, r4
/* 802CBC6C 0029522C  20 06 00 03 */	subfic r0, r6, 0x3
/* 802CBC70 00295230  81 0C 00 00 */	lwz r8, 0x0(r12)
/* 802CBC74 00295234  54 00 18 38 */	slwi r0, r0, 3
/* 802CBC78 00295238  55 5F 18 38 */	slwi r31, r10, 3
/* 802CBC7C 0029523C  7C E0 04 30 */	srw r0, r7, r0
/* 802CBC80 00295240  38 84 00 01 */	addi r4, r4, 0x1
/* 802CBC84 00295244  54 00 06 3E */	clrlwi r0, r0, 24
/* 802CBC88 00295248  7D 2A F8 30 */	slw r10, r9, r31
/* 802CBC8C 0029524C  7C 00 F8 30 */	slw r0, r0, r31
/* 802CBC90 00295250  3B DE 00 01 */	addi r30, r30, 0x1
/* 802CBC94 00295254  7D 07 50 78 */	andc r7, r8, r10
/* 802CBC98 00295258  7D 40 00 38 */	and r0, r10, r0
/* 802CBC9C 0029525C  7C E0 03 78 */	or r0, r7, r0
/* 802CBCA0 00295260  90 0C 00 00 */	stw r0, 0x0(r12)
/* 802CBCA4 00295264  42 00 FF B0 */	bdnz .L_802CBC54
.L_802CBCA8:
/* 802CBCA8 00295268  83 E1 00 0C */	lwz r31, 0xc(r1)
/* 802CBCAC 0029526C  83 C1 00 08 */	lwz r30, 0x8(r1)
/* 802CBCB0 00295270  38 21 00 10 */	addi r1, r1, 0x10
/* 802CBCB4 00295274  4E 80 00 20 */	blr
.endfn TRK_memcpy

.fn TRK_memset, global
/* 802CBCB8 00295278  2C 05 00 00 */	cmpwi r5, 0x0
/* 802CBCBC 0029527C  54 88 06 3E */	clrlwi r8, r4, 24
/* 802CBCC0 00295280  7C 67 1B 78 */	mr r7, r3
/* 802CBCC4 00295284  38 C0 00 FF */	li r6, 0xff
/* 802CBCC8 00295288  4D 82 00 20 */	beqlr
/* 802CBCCC 0029528C  54 A0 F0 BF */	srwi. r0, r5, 2
/* 802CBCD0 00295290  7C 09 03 A6 */	mtctr r0
/* 802CBCD4 00295294  41 82 00 D0 */	beq .L_802CBDA4
.L_802CBCD8:
/* 802CBCD8 00295298  54 EA 00 3A */	clrrwi r10, r7, 2
/* 802CBCDC 0029529C  7C 0A 38 50 */	subf r0, r10, r7
/* 802CBCE0 002952A0  80 8A 00 00 */	lwz r4, 0x0(r10)
/* 802CBCE4 002952A4  20 00 00 03 */	subfic r0, r0, 0x3
/* 802CBCE8 002952A8  38 E7 00 01 */	addi r7, r7, 0x1
/* 802CBCEC 002952AC  54 00 18 38 */	slwi r0, r0, 3
/* 802CBCF0 002952B0  7C C9 00 30 */	slw r9, r6, r0
/* 802CBCF4 002952B4  7D 00 00 30 */	slw r0, r8, r0
/* 802CBCF8 002952B8  7C 84 48 78 */	andc r4, r4, r9
/* 802CBCFC 002952BC  7D 20 00 38 */	and r0, r9, r0
/* 802CBD00 002952C0  7C 80 03 78 */	or r0, r4, r0
/* 802CBD04 002952C4  90 0A 00 00 */	stw r0, 0x0(r10)
/* 802CBD08 002952C8  54 EA 00 3A */	clrrwi r10, r7, 2
/* 802CBD0C 002952CC  7C 0A 38 50 */	subf r0, r10, r7
/* 802CBD10 002952D0  80 8A 00 00 */	lwz r4, 0x0(r10)
/* 802CBD14 002952D4  20 00 00 03 */	subfic r0, r0, 0x3
/* 802CBD18 002952D8  38 E7 00 01 */	addi r7, r7, 0x1
/* 802CBD1C 002952DC  54 00 18 38 */	slwi r0, r0, 3
/* 802CBD20 002952E0  7C C9 00 30 */	slw r9, r6, r0
/* 802CBD24 002952E4  7D 00 00 30 */	slw r0, r8, r0
/* 802CBD28 002952E8  7C 84 48 78 */	andc r4, r4, r9
/* 802CBD2C 002952EC  7D 20 00 38 */	and r0, r9, r0
/* 802CBD30 002952F0  7C 80 03 78 */	or r0, r4, r0
/* 802CBD34 002952F4  90 0A 00 00 */	stw r0, 0x0(r10)
/* 802CBD38 002952F8  54 EA 00 3A */	clrrwi r10, r7, 2
/* 802CBD3C 002952FC  7C 0A 38 50 */	subf r0, r10, r7
/* 802CBD40 00295300  80 8A 00 00 */	lwz r4, 0x0(r10)
/* 802CBD44 00295304  20 00 00 03 */	subfic r0, r0, 0x3
/* 802CBD48 00295308  38 E7 00 01 */	addi r7, r7, 0x1
/* 802CBD4C 0029530C  54 00 18 38 */	slwi r0, r0, 3
/* 802CBD50 00295310  7C C9 00 30 */	slw r9, r6, r0
/* 802CBD54 00295314  7D 00 00 30 */	slw r0, r8, r0
/* 802CBD58 00295318  7C 84 48 78 */	andc r4, r4, r9
/* 802CBD5C 0029531C  7D 20 00 38 */	and r0, r9, r0
/* 802CBD60 00295320  7C 80 03 78 */	or r0, r4, r0
/* 802CBD64 00295324  90 0A 00 00 */	stw r0, 0x0(r10)
/* 802CBD68 00295328  54 EA 00 3A */	clrrwi r10, r7, 2
/* 802CBD6C 0029532C  7C 0A 38 50 */	subf r0, r10, r7
/* 802CBD70 00295330  80 8A 00 00 */	lwz r4, 0x0(r10)
/* 802CBD74 00295334  20 00 00 03 */	subfic r0, r0, 0x3
/* 802CBD78 00295338  38 E7 00 01 */	addi r7, r7, 0x1
/* 802CBD7C 0029533C  54 00 18 38 */	slwi r0, r0, 3
/* 802CBD80 00295340  7C C9 00 30 */	slw r9, r6, r0
/* 802CBD84 00295344  7D 00 00 30 */	slw r0, r8, r0
/* 802CBD88 00295348  7C 84 48 78 */	andc r4, r4, r9
/* 802CBD8C 0029534C  7D 20 00 38 */	and r0, r9, r0
/* 802CBD90 00295350  7C 80 03 78 */	or r0, r4, r0
/* 802CBD94 00295354  90 0A 00 00 */	stw r0, 0x0(r10)
/* 802CBD98 00295358  42 00 FF 40 */	bdnz .L_802CBCD8
/* 802CBD9C 0029535C  70 A5 00 03 */	andi. r5, r5, 0x3
/* 802CBDA0 00295360  4D 82 00 20 */	beqlr
.L_802CBDA4:
/* 802CBDA4 00295364  7C A9 03 A6 */	mtctr r5
.L_802CBDA8:
/* 802CBDA8 00295368  54 EA 00 3A */	clrrwi r10, r7, 2
/* 802CBDAC 0029536C  7C 0A 38 50 */	subf r0, r10, r7
/* 802CBDB0 00295370  80 8A 00 00 */	lwz r4, 0x0(r10)
/* 802CBDB4 00295374  20 00 00 03 */	subfic r0, r0, 0x3
/* 802CBDB8 00295378  38 E7 00 01 */	addi r7, r7, 0x1
/* 802CBDBC 0029537C  54 00 18 38 */	slwi r0, r0, 3
/* 802CBDC0 00295380  7C C9 00 30 */	slw r9, r6, r0
/* 802CBDC4 00295384  7D 00 00 30 */	slw r0, r8, r0
/* 802CBDC8 00295388  7C 84 48 78 */	andc r4, r4, r9
/* 802CBDCC 0029538C  7D 20 00 38 */	and r0, r9, r0
/* 802CBDD0 00295390  7C 80 03 78 */	or r0, r4, r0
/* 802CBDD4 00295394  90 0A 00 00 */	stw r0, 0x0(r10)
/* 802CBDD8 00295398  42 00 FF D0 */	bdnz .L_802CBDA8
/* 802CBDDC 0029539C  4E 80 00 20 */	blr
.endfn TRK_memset
