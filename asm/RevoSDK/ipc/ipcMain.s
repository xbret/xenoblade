.include "macros.inc"

.section .text, "ax"  # 0x80039220 - 0x804F5900

.balign 16, 0
.fn IPCInit, global
/* 803429C0 0030BF80  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 803429C4 0030BF84  7C 08 02 A6 */	mflr r0
/* 803429C8 0030BF88  90 01 00 14 */	stw r0, 0x14(r1)
/* 803429CC 0030BF8C  88 0D B7 E8 */	lbz r0, Initialized@sda21(r13)
/* 803429D0 0030BF90  2C 00 00 00 */	cmpwi r0, 0
/* 803429D4 0030BF94  40 82 00 28 */	bne .L_803429FC
/* 803429D8 0030BF98  48 01 A6 09 */	bl __OSGetIPCBufferHi
/* 803429DC 0030BF9C  90 6D B7 F8 */	stw r3, IPCBufferHi@sda21(r13)
/* 803429E0 0030BFA0  48 01 A6 11 */	bl __OSGetIPCBufferLo
/* 803429E4 0030BFA4  80 8D B7 F8 */	lwz r4, IPCBufferHi@sda21(r13)
/* 803429E8 0030BFA8  38 00 00 01 */	li r0, 1
/* 803429EC 0030BFAC  90 6D B7 F4 */	stw r3, IPCBufferLo@sda21(r13)
/* 803429F0 0030BFB0  90 8D B7 F0 */	stw r4, IPCCurrentBufferHi@sda21(r13)
/* 803429F4 0030BFB4  90 6D B7 EC */	stw r3, IPCCurrentBufferLo@sda21(r13)
/* 803429F8 0030BFB8  98 0D B7 E8 */	stb r0, Initialized@sda21(r13)
.L_803429FC:
/* 803429FC 0030BFBC  80 01 00 14 */	lwz r0, 0x14(r1)
/* 80342A00 0030BFC0  7C 08 03 A6 */	mtlr r0
/* 80342A04 0030BFC4  38 21 00 10 */	addi r1, r1, 0x10
/* 80342A08 0030BFC8  4E 80 00 20 */	blr 
.endfn IPCInit

.balign 16, 0
.fn IPCReInit, global
/* 80342A10 0030BFD0  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 80342A14 0030BFD4  7C 08 02 A6 */	mflr r0
/* 80342A18 0030BFD8  90 01 00 14 */	stw r0, 0x14(r1)
/* 80342A1C 0030BFDC  38 00 00 00 */	li r0, 0
/* 80342A20 0030BFE0  98 0D B7 E8 */	stb r0, Initialized@sda21(r13)
/* 80342A24 0030BFE4  48 01 A5 BD */	bl __OSGetIPCBufferHi
/* 80342A28 0030BFE8  90 6D B7 F8 */	stw r3, IPCBufferHi@sda21(r13)
/* 80342A2C 0030BFEC  48 01 A5 C5 */	bl __OSGetIPCBufferLo
/* 80342A30 0030BFF0  80 8D B7 F8 */	lwz r4, IPCBufferHi@sda21(r13)
/* 80342A34 0030BFF4  38 00 00 01 */	li r0, 1
/* 80342A38 0030BFF8  90 6D B7 F4 */	stw r3, IPCBufferLo@sda21(r13)
/* 80342A3C 0030BFFC  90 8D B7 F0 */	stw r4, IPCCurrentBufferHi@sda21(r13)
/* 80342A40 0030C000  90 6D B7 EC */	stw r3, IPCCurrentBufferLo@sda21(r13)
/* 80342A44 0030C004  98 0D B7 E8 */	stb r0, Initialized@sda21(r13)
/* 80342A48 0030C008  80 01 00 14 */	lwz r0, 0x14(r1)
/* 80342A4C 0030C00C  7C 08 03 A6 */	mtlr r0
/* 80342A50 0030C010  38 21 00 10 */	addi r1, r1, 0x10
/* 80342A54 0030C014  4E 80 00 20 */	blr 
.endfn IPCReInit

.balign 16, 0
.fn IPCReadReg, global
/* 80342A60 0030C020  54 60 10 3A */	slwi r0, r3, 2
/* 80342A64 0030C024  3C 60 CD 00 */	lis r3, 0xcd00
/* 80342A68 0030C028  7C 63 00 2E */	lwzx r3, r3, r0
/* 80342A6C 0030C02C  4E 80 00 20 */	blr 
.endfn IPCReadReg

.balign 16, 0
.fn IPCWriteReg, global
/* 80342A70 0030C030  54 60 10 3A */	slwi r0, r3, 2
/* 80342A74 0030C034  3C 60 CD 00 */	lis r3, 0xcd00
/* 80342A78 0030C038  7C 83 01 2E */	stwx r4, r3, r0
/* 80342A7C 0030C03C  4E 80 00 20 */	blr 
.endfn IPCWriteReg

.balign 16, 0
.fn IPCGetBufferHi, global
/* 80342A80 0030C040  80 6D B7 F0 */	lwz r3, IPCCurrentBufferHi@sda21(r13)
/* 80342A84 0030C044  4E 80 00 20 */	blr 
.endfn IPCGetBufferHi

.balign 16, 0
.fn IPCGetBufferLo, global
/* 80342A90 0030C050  80 6D B7 EC */	lwz r3, IPCCurrentBufferLo@sda21(r13)
/* 80342A94 0030C054  4E 80 00 20 */	blr 
.endfn IPCGetBufferLo

.balign 16, 0
.fn IPCSetBufferLo, global
/* 80342AA0 0030C060  90 6D B7 EC */	stw r3, IPCCurrentBufferLo@sda21(r13)
/* 80342AA4 0030C064  4E 80 00 20 */	blr 
.endfn IPCSetBufferLo

.section .sbss, "wa"  # 0x80666600 - 0x8066836F

.balign 8

.obj Initialized, local
	.skip 0x4
.endobj Initialized

.obj IPCCurrentBufferLo, local
	.skip 0x4
.endobj IPCCurrentBufferLo

.obj IPCCurrentBufferHi, local
	.skip 0x4
.endobj IPCCurrentBufferHi

.obj IPCBufferLo, local
	.skip 0x4
.endobj IPCBufferLo

.obj IPCBufferHi, local
	.skip 0x8
.endobj IPCBufferHi
