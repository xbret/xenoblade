.include "macros.inc"
.file "kyoshin/appgame/cf/CfObjectObj.o"

# 0x80009F18 - 0x80009F70
.section extab, "a"
.balign 4

.obj "@etb_80009F18", local
.hidden "@etb_80009F18"
	.4byte 0x08080000
	.4byte 0x00000044
	.4byte 0x000A0010
	.4byte 0x00000000
	.4byte 0x8680001F
	.4byte 0x00000000
	.4byte fn_800BBD6C
.endobj "@etb_80009F18"

.obj "@etb_80009F34", local
.hidden "@etb_80009F34"
	.4byte 0x10080000
	.4byte 0x0000003C
	.4byte 0x00000010
	.4byte 0x00000000
	.4byte 0x8680001E
	.4byte 0x00000000
	.4byte fn_800BBD6C
.endobj "@etb_80009F34"

.obj "@etb_80009F50", local
.hidden "@etb_80009F50"
	.4byte 0x00080000
	.4byte 0x00000000
.endobj "@etb_80009F50"

.obj "@etb_80009F58", local
.hidden "@etb_80009F58"
	.4byte 0x18080000
	.4byte 0x00000000
.endobj "@etb_80009F58"

.obj "@etb_80009F60", local
.hidden "@etb_80009F60"
	.4byte 0x08080000
	.4byte 0x00000000
.endobj "@etb_80009F60"

.obj "@etb_80009F68", local
.hidden "@etb_80009F68"
	.4byte 0x10080000
	.4byte 0x00000000
.endobj "@etb_80009F68"

# 0x80025A30 - 0x80025A78
.section extabindex, "a"
.balign 4

.obj "@eti_80025A30", local
.hidden "@eti_80025A30"
	.4byte fn_800BF98C
	.4byte 0x00000088
	.4byte "@etb_80009F18"
.endobj "@eti_80025A30"

.obj "@eti_80025A3C", local
.hidden "@eti_80025A3C"
	.4byte fn_800BFA14
	.4byte 0x00000074
	.4byte "@etb_80009F34"
.endobj "@eti_80025A3C"

.obj "@eti_80025A48", local
.hidden "@eti_80025A48"
	.4byte fn_800BFA88
	.4byte 0x00000024
	.4byte "@etb_80009F50"
.endobj "@eti_80025A48"

.obj "@eti_80025A54", local
.hidden "@eti_80025A54"
	.4byte fn_800BFAB0
	.4byte 0x000000E0
	.4byte "@etb_80009F58"
.endobj "@eti_80025A54"

.obj "@eti_80025A60", local
.hidden "@eti_80025A60"
	.4byte fn_800BFB90
	.4byte 0x00000064
	.4byte "@etb_80009F60"
.endobj "@eti_80025A60"

.obj "@eti_80025A6C", local
.hidden "@eti_80025A6C"
	.4byte fn_800BFBF4
	.4byte 0x00000074
	.4byte "@etb_80009F68"
.endobj "@eti_80025A6C"

# 0x800BF98C - 0x800BFC90
.text
.balign 4

.fn fn_800BF98C, local
/* 800BF98C 00088F4C  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 800BF990 00088F50  7C 08 02 A6 */	mflr r0
/* 800BF994 00088F54  90 01 00 14 */	stw r0, 0x14(r1)
/* 800BF998 00088F58  93 E1 00 0C */	stw r31, 0xc(r1)
/* 800BF99C 00088F5C  7C 7F 1B 78 */	mr r31, r3
/* 800BF9A0 00088F60  4B FF C2 AD */	bl fn_800BBC4C
/* 800BF9A4 00088F64  3C 60 80 53 */	lis r3, __vt__cf_CfObjectObj@ha
/* 800BF9A8 00088F68  38 00 00 00 */	li r0, 0x0
/* 800BF9AC 00088F6C  38 63 D3 1C */	addi r3, r3, __vt__cf_CfObjectObj@l
/* 800BF9B0 00088F70  90 7F 00 00 */	stw r3, 0x0(r31)
/* 800BF9B4 00088F74  7F E3 FB 78 */	mr r3, r31
/* 800BF9B8 00088F78  38 80 00 00 */	li r4, 0x0
/* 800BF9BC 00088F7C  B0 1F 07 16 */	sth r0, 0x716(r31)
/* 800BF9C0 00088F80  38 AD 84 90 */	addi r5, r13, __RTTI__cf_CfObjectTbox@sda21
/* 800BF9C4 00088F84  38 CD 84 98 */	addi r6, r13, __RTTI__cf_CfObjectObj@sda21
/* 800BF9C8 00088F88  38 E0 00 00 */	li r7, 0x0
/* 800BF9CC 00088F8C  48 1F A3 B9 */	bl __dynamic_cast
/* 800BF9D0 00088F90  2C 03 00 00 */	cmpwi r3, 0x0
/* 800BF9D4 00088F94  40 82 00 28 */	bne .L_800BF9FC
/* 800BF9D8 00088F98  4B FA 26 25 */	bl fn_80061FFC
/* 800BF9DC 00088F9C  7C 64 1B 78 */	mr r4, r3
/* 800BF9E0 00088FA0  38 60 00 1C */	li r3, 0x1c
/* 800BF9E4 00088FA4  48 37 50 79 */	bl mm_malloc
/* 800BF9E8 00088FA8  2C 03 00 00 */	cmpwi r3, 0x0
/* 800BF9EC 00088FAC  41 82 00 0C */	beq .L_800BF9F8
/* 800BF9F0 00088FB0  7F E4 FB 78 */	mr r4, r31
/* 800BF9F4 00088FB4  48 0A CD FD */	bl fn_8016C7F0
.L_800BF9F8:
/* 800BF9F8 00088FB8  90 7F 00 B0 */	stw r3, 0xb0(r31)
.L_800BF9FC:
/* 800BF9FC 00088FBC  7F E3 FB 78 */	mr r3, r31
/* 800BFA00 00088FC0  83 E1 00 0C */	lwz r31, 0xc(r1)
/* 800BFA04 00088FC4  80 01 00 14 */	lwz r0, 0x14(r1)
/* 800BFA08 00088FC8  7C 08 03 A6 */	mtlr r0
/* 800BFA0C 00088FCC  38 21 00 10 */	addi r1, r1, 0x10
/* 800BFA10 00088FD0  4E 80 00 20 */	blr
.endfn fn_800BF98C

.fn fn_800BFA14, local
/* 800BFA14 00088FD4  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 800BFA18 00088FD8  7C 08 02 A6 */	mflr r0
/* 800BFA1C 00088FDC  2C 03 00 00 */	cmpwi r3, 0x0
/* 800BFA20 00088FE0  90 01 00 14 */	stw r0, 0x14(r1)
/* 800BFA24 00088FE4  93 E1 00 0C */	stw r31, 0xc(r1)
/* 800BFA28 00088FE8  7C 9F 23 78 */	mr r31, r4
/* 800BFA2C 00088FEC  93 C1 00 08 */	stw r30, 0x8(r1)
/* 800BFA30 00088FF0  7C 7E 1B 78 */	mr r30, r3
/* 800BFA34 00088FF4  41 82 00 38 */	beq .L_800BFA6C
/* 800BFA38 00088FF8  3D 80 80 53 */	lis r12, __vt__cf_CfObjectObj@ha
/* 800BFA3C 00088FFC  39 8C D3 1C */	addi r12, r12, __vt__cf_CfObjectObj@l
/* 800BFA40 00089000  91 83 00 00 */	stw r12, 0x0(r3)
/* 800BFA44 00089004  81 8C 00 68 */	lwz r12, 0x68(r12)
/* 800BFA48 00089008  7D 89 03 A6 */	mtctr r12
/* 800BFA4C 0008900C  4E 80 04 21 */	bctrl
/* 800BFA50 00089010  7F C3 F3 78 */	mr r3, r30
/* 800BFA54 00089014  38 80 00 00 */	li r4, 0x0
/* 800BFA58 00089018  4B FF C3 15 */	bl fn_800BBD6C
/* 800BFA5C 0008901C  2C 1F 00 00 */	cmpwi r31, 0x0
/* 800BFA60 00089020  40 81 00 0C */	ble .L_800BFA6C
/* 800BFA64 00089024  7F C3 F3 78 */	mr r3, r30
/* 800BFA68 00089028  48 37 51 C5 */	bl __dl__FPv
.L_800BFA6C:
/* 800BFA6C 0008902C  7F C3 F3 78 */	mr r3, r30
/* 800BFA70 00089030  83 E1 00 0C */	lwz r31, 0xc(r1)
/* 800BFA74 00089034  83 C1 00 08 */	lwz r30, 0x8(r1)
/* 800BFA78 00089038  80 01 00 14 */	lwz r0, 0x14(r1)
/* 800BFA7C 0008903C  7C 08 03 A6 */	mtlr r0
/* 800BFA80 00089040  38 21 00 10 */	addi r1, r1, 0x10
/* 800BFA84 00089044  4E 80 00 20 */	blr
.endfn fn_800BFA14

.fn fn_800BFA88, local
/* 800BFA88 00089048  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 800BFA8C 0008904C  7C 08 02 A6 */	mflr r0
/* 800BFA90 00089050  90 01 00 14 */	stw r0, 0x14(r1)
/* 800BFA94 00089054  4B FF AE 6D */	bl fn_800BA900
/* 800BFA98 00089058  80 01 00 14 */	lwz r0, 0x14(r1)
/* 800BFA9C 0008905C  38 60 00 01 */	li r3, 0x1
/* 800BFAA0 00089060  7C 08 03 A6 */	mtlr r0
/* 800BFAA4 00089064  38 21 00 10 */	addi r1, r1, 0x10
/* 800BFAA8 00089068  4E 80 00 20 */	blr
.endfn fn_800BFA88

.fn fn_800BFAAC, local
/* 800BFAAC 0008906C  4E 80 00 20 */	blr
.endfn fn_800BFAAC

.fn fn_800BFAB0, local
/* 800BFAB0 00089070  94 21 FF E0 */	stwu r1, -0x20(r1)
/* 800BFAB4 00089074  7C 08 02 A6 */	mflr r0
/* 800BFAB8 00089078  90 01 00 24 */	stw r0, 0x24(r1)
/* 800BFABC 0008907C  93 E1 00 1C */	stw r31, 0x1c(r1)
/* 800BFAC0 00089080  7C BF 2B 78 */	mr r31, r5
/* 800BFAC4 00089084  93 C1 00 18 */	stw r30, 0x18(r1)
/* 800BFAC8 00089088  7C 9E 23 78 */	mr r30, r4
/* 800BFACC 0008908C  93 A1 00 14 */	stw r29, 0x14(r1)
/* 800BFAD0 00089090  7C 7D 1B 78 */	mr r29, r3
/* 800BFAD4 00089094  81 83 00 00 */	lwz r12, 0x0(r3)
/* 800BFAD8 00089098  81 8C 01 7C */	lwz r12, 0x17c(r12)
/* 800BFADC 0008909C  7D 89 03 A6 */	mtctr r12
/* 800BFAE0 000890A0  4E 80 04 21 */	bctrl
/* 800BFAE4 000890A4  81 9D 00 00 */	lwz r12, 0x0(r29)
/* 800BFAE8 000890A8  7F A3 EB 78 */	mr r3, r29
/* 800BFAEC 000890AC  81 8C 01 78 */	lwz r12, 0x178(r12)
/* 800BFAF0 000890B0  7D 89 03 A6 */	mtctr r12
/* 800BFAF4 000890B4  4E 80 04 21 */	bctrl
/* 800BFAF8 000890B8  38 00 00 00 */	li r0, 0x0
/* 800BFAFC 000890BC  90 1D 00 90 */	stw r0, 0x90(r29)
/* 800BFB00 000890C0  7F A3 EB 78 */	mr r3, r29
/* 800BFB04 000890C4  7F C5 F3 78 */	mr r5, r30
/* 800BFB08 000890C8  90 1D 00 94 */	stw r0, 0x94(r29)
/* 800BFB0C 000890CC  38 80 00 01 */	li r4, 0x1
/* 800BFB10 000890D0  81 9D 00 00 */	lwz r12, 0x0(r29)
/* 800BFB14 000890D4  81 8C 01 44 */	lwz r12, 0x144(r12)
/* 800BFB18 000890D8  7D 89 03 A6 */	mtctr r12
/* 800BFB1C 000890DC  4E 80 04 21 */	bctrl
/* 800BFB20 000890E0  81 9D 00 00 */	lwz r12, 0x0(r29)
/* 800BFB24 000890E4  7F A3 EB 78 */	mr r3, r29
/* 800BFB28 000890E8  7F E5 FB 78 */	mr r5, r31
/* 800BFB2C 000890EC  38 80 00 00 */	li r4, 0x0
/* 800BFB30 000890F0  81 8C 01 44 */	lwz r12, 0x144(r12)
/* 800BFB34 000890F4  7D 89 03 A6 */	mtctr r12
/* 800BFB38 000890F8  4E 80 04 21 */	bctrl
/* 800BFB3C 000890FC  80 7D 00 B0 */	lwz r3, 0xb0(r29)
/* 800BFB40 00089100  38 00 00 01 */	li r0, 0x1
/* 800BFB44 00089104  2C 1E 00 00 */	cmpwi r30, 0x0
/* 800BFB48 00089108  B0 03 00 08 */	sth r0, 0x8(r3)
/* 800BFB4C 0008910C  41 82 00 10 */	beq .L_800BFB5C
/* 800BFB50 00089110  80 1D 00 6C */	lwz r0, 0x6c(r29)
/* 800BFB54 00089114  60 00 00 20 */	ori r0, r0, 0x20
/* 800BFB58 00089118  90 1D 00 6C */	stw r0, 0x6c(r29)
.L_800BFB5C:
/* 800BFB5C 0008911C  2C 1F 00 00 */	cmpwi r31, 0x0
/* 800BFB60 00089120  41 82 00 10 */	beq .L_800BFB70
/* 800BFB64 00089124  80 1D 00 6C */	lwz r0, 0x6c(r29)
/* 800BFB68 00089128  60 00 00 10 */	ori r0, r0, 0x10
/* 800BFB6C 0008912C  90 1D 00 6C */	stw r0, 0x6c(r29)
.L_800BFB70:
/* 800BFB70 00089130  83 E1 00 1C */	lwz r31, 0x1c(r1)
/* 800BFB74 00089134  38 60 00 01 */	li r3, 0x1
/* 800BFB78 00089138  83 C1 00 18 */	lwz r30, 0x18(r1)
/* 800BFB7C 0008913C  83 A1 00 14 */	lwz r29, 0x14(r1)
/* 800BFB80 00089140  80 01 00 24 */	lwz r0, 0x24(r1)
/* 800BFB84 00089144  7C 08 03 A6 */	mtlr r0
/* 800BFB88 00089148  38 21 00 20 */	addi r1, r1, 0x20
/* 800BFB8C 0008914C  4E 80 00 20 */	blr
.endfn fn_800BFAB0

.fn fn_800BFB90, local
/* 800BFB90 00089150  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 800BFB94 00089154  7C 08 02 A6 */	mflr r0
/* 800BFB98 00089158  90 01 00 14 */	stw r0, 0x14(r1)
/* 800BFB9C 0008915C  93 E1 00 0C */	stw r31, 0xc(r1)
/* 800BFBA0 00089160  7C 7F 1B 78 */	mr r31, r3
/* 800BFBA4 00089164  4B FF C2 69 */	bl fn_800BBE0C
/* 800BFBA8 00089168  81 9F 00 00 */	lwz r12, 0x0(r31)
/* 800BFBAC 0008916C  7F E3 FB 78 */	mr r3, r31
/* 800BFBB0 00089170  81 8C 00 74 */	lwz r12, 0x74(r12)
/* 800BFBB4 00089174  7D 89 03 A6 */	mtctr r12
/* 800BFBB8 00089178  4E 80 04 21 */	bctrl
/* 800BFBBC 0008917C  2C 03 00 00 */	cmpwi r3, 0x0
/* 800BFBC0 00089180  41 82 00 20 */	beq .L_800BFBE0
/* 800BFBC4 00089184  A0 9F 07 16 */	lhz r4, 0x716(r31)
/* 800BFBC8 00089188  2C 04 00 00 */	cmpwi r4, 0x0
/* 800BFBCC 0008918C  41 82 00 14 */	beq .L_800BFBE0
/* 800BFBD0 00089190  80 7F 00 38 */	lwz r3, 0x38(r31)
/* 800BFBD4 00089194  48 00 A9 AD */	bl fn_800CA580
/* 800BFBD8 00089198  38 00 00 00 */	li r0, 0x0
/* 800BFBDC 0008919C  B0 1F 07 16 */	sth r0, 0x716(r31)
.L_800BFBE0:
/* 800BFBE0 000891A0  80 01 00 14 */	lwz r0, 0x14(r1)
/* 800BFBE4 000891A4  83 E1 00 0C */	lwz r31, 0xc(r1)
/* 800BFBE8 000891A8  7C 08 03 A6 */	mtlr r0
/* 800BFBEC 000891AC  38 21 00 10 */	addi r1, r1, 0x10
/* 800BFBF0 000891B0  4E 80 00 20 */	blr
.endfn fn_800BFB90

.fn fn_800BFBF4, local
/* 800BFBF4 000891B4  94 21 FF F0 */	stwu r1, -0x10(r1)
/* 800BFBF8 000891B8  7C 08 02 A6 */	mflr r0
/* 800BFBFC 000891BC  90 01 00 14 */	stw r0, 0x14(r1)
/* 800BFC00 000891C0  93 E1 00 0C */	stw r31, 0xc(r1)
/* 800BFC04 000891C4  7C 9F 23 78 */	mr r31, r4
/* 800BFC08 000891C8  93 C1 00 08 */	stw r30, 0x8(r1)
/* 800BFC0C 000891CC  7C 7E 1B 78 */	mr r30, r3
/* 800BFC10 000891D0  80 03 00 38 */	lwz r0, 0x38(r3)
/* 800BFC14 000891D4  2C 00 00 00 */	cmpwi r0, 0x0
/* 800BFC18 000891D8  41 82 00 38 */	beq .L_800BFC50
/* 800BFC1C 000891DC  81 83 00 00 */	lwz r12, 0x0(r3)
/* 800BFC20 000891E0  81 8C 00 74 */	lwz r12, 0x74(r12)
/* 800BFC24 000891E4  7D 89 03 A6 */	mtctr r12
/* 800BFC28 000891E8  4E 80 04 21 */	bctrl
/* 800BFC2C 000891EC  2C 03 00 00 */	cmpwi r3, 0x0
/* 800BFC30 000891F0  41 82 00 1C */	beq .L_800BFC4C
/* 800BFC34 000891F4  80 7E 00 38 */	lwz r3, 0x38(r30)
/* 800BFC38 000891F8  7F E4 FB 78 */	mr r4, r31
/* 800BFC3C 000891FC  48 00 A9 45 */	bl fn_800CA580
/* 800BFC40 00089200  38 00 00 00 */	li r0, 0x0
/* 800BFC44 00089204  B0 1E 07 16 */	sth r0, 0x716(r30)
/* 800BFC48 00089208  48 00 00 08 */	b .L_800BFC50
.L_800BFC4C:
/* 800BFC4C 0008920C  B3 FE 07 16 */	sth r31, 0x716(r30)
.L_800BFC50:
/* 800BFC50 00089210  80 01 00 14 */	lwz r0, 0x14(r1)
/* 800BFC54 00089214  83 E1 00 0C */	lwz r31, 0xc(r1)
/* 800BFC58 00089218  83 C1 00 08 */	lwz r30, 0x8(r1)
/* 800BFC5C 0008921C  7C 08 03 A6 */	mtlr r0
/* 800BFC60 00089220  38 21 00 10 */	addi r1, r1, 0x10
/* 800BFC64 00089224  4E 80 00 20 */	blr
.endfn fn_800BFBF4

.fn fn_800BFC68, local
/* 800BFC68 00089228  2C 83 00 00 */	cmpwi cr1, r3, 0x0
/* 800BFC6C 0008922C  41 86 00 1C */	beq cr1, .L_800BFC88
/* 800BFC70 00089230  80 03 00 64 */	lwz r0, 0x64(r3)
/* 800BFC74 00089234  54 00 07 BD */	rlwinm. r0, r0, 0, 30, 30
/* 800BFC78 00089238  41 82 00 10 */	beq .L_800BFC88
/* 800BFC7C 0008923C  4D 86 00 20 */	beqlr cr1
/* 800BFC80 00089240  38 63 C1 64 */	addi r3, r3, -0x3e9c
/* 800BFC84 00089244  4E 80 00 20 */	blr
.L_800BFC88:
/* 800BFC88 00089248  38 60 00 00 */	li r3, 0x0
/* 800BFC8C 0008924C  4E 80 00 20 */	blr
.endfn fn_800BFC68

# 0x804F7DD8 - 0x804F7E00
.rodata
.balign 8

.obj cf_CfObjectTbox_typestr, global
	.4byte 0x63663A3A
	.4byte 0x43664F62
	.4byte 0x6A656374
	.4byte 0x54626F78
	.4byte 0x00000000
	.4byte 0x00000000

.obj cf_CfObjectObj_typestr, global
	.4byte 0x63663A3A
	.4byte 0x43664F62
	.4byte 0x6A656374
	.4byte 0x4F626A00

# 0x8052D2E8 - 0x8052D570
.data
.balign 8

.obj cf_CfObjectTbox_hierarchy, global
	.4byte __RTTI__cf_CObjectState
	.4byte 0x00000000
	.4byte __RTTI__cf_CObjectParam
	.4byte 0x00000000
	.4byte __RTTI__cf_CfObject
	.4byte 0x00000000
	.4byte __RTTI__cf_CfObjectModel
	.4byte 0x00000000
	.4byte __RTTI__cf_CfObjectMove
	.4byte 0x00000000
	.4byte __RTTI__cf_CfObjectObj
	.4byte 0x00000000
	.4byte 0x00000000

.obj __vt__cf_CfObjectObj, global
	.4byte __RTTI__cf_CfObjectObj
	.4byte 0x00000000
	.4byte fn_8003DC64
	.4byte fn_8003CB58
	.4byte fn_8003DD34
	.4byte fn_800A34A8
	.4byte fn_8009489C
	.4byte fn_800A349C
	.4byte fn_80086B4C
	.4byte fn_8003CB20
	.4byte fn_8003D98C
	.4byte fn_8003D8C0
	.4byte fn_8003D7B0
	.4byte fn_800A3494
	.4byte fn_800A3488
	.4byte fn_8003C7F8
	.4byte fn_800BE0F4
	.4byte fn_800A34B4
	.4byte fn_800BFAAC
	.4byte fn_8003E664
	.4byte fn_8003DC74
	.4byte fn_800BFA14
	.4byte fn_800BFA88
	.4byte fn_800AC75C
	.4byte fn_800BFB90
	.4byte fn_800BC758
	.4byte fn_800BC040
	.4byte fn_800BC020
	.4byte fn_800BABBC
	.4byte fn_800BE984
	.4byte fn_800BE998
	.4byte fn_800AC5E4
	.4byte fn_800BE9E4
	.4byte fn_800BE088
	.4byte fn_800BE1C4
	.4byte fn_800BE1F8
	.4byte fn_800BE214
	.4byte fn_800BE248
	.4byte fn_8008F9CC
	.4byte fn_800BDB6C
	.4byte fn_800BB0E8
	.4byte fn_800AC5A8
	.4byte fn_800BDBC0
	.4byte fn_800BDE8C
	.4byte fn_800BB168
	.4byte fn_800BDC24
	.4byte fn_800BDD58
	.4byte fn_800BDEC4
	.4byte fn_800BB200
	.4byte fn_800BDF78
	.4byte fn_800BDFE0
	.4byte fn_800BB290
	.4byte fn_800BE058
	.4byte fn_800BE070
	.4byte fn_800BB2E4
	.4byte fn_800BB318
	.4byte fn_800BB338
	.4byte fn_800BEC4C
	.4byte fn_800BECF8
	.4byte fn_800BEE34
	.4byte fn_800BEF20
	.4byte fn_800AC570
	.4byte fn_800BF014
	.4byte fn_800BF100
	.4byte fn_800AC558
	.4byte fn_800BF1F4
	.4byte fn_800BD774
	.4byte fn_800BD860
	.4byte fn_8004ABD8
	.4byte fn_800BDA10
	.4byte fn_8003BD64
	.4byte fn_8003BD70
	.4byte fn_800BB3AC
	.4byte fn_800BB454
	.4byte fn_800BB4FC
	.4byte fn_800BB534
	.4byte fn_800BB364
	.4byte fn_800BE0B8
	.4byte fn_8008E064
	.4byte fn_800AEAE4
	.4byte fn_80087EE4
	.4byte fn_800BE9BC
	.4byte fn_800BE9D0
	.4byte fn_800BACDC
	.4byte fn_800BDA9C
	.4byte fn_800BDAF4
	.4byte fn_800BEB28
	.4byte fn_800BB684
	.4byte fn_800BB974
	.4byte fn_800BB954
	.4byte fn_800BB708
	.4byte fn_80094FF8
	.4byte fn_800BB730
	.4byte fn_80094FF0
	.4byte fn_800BC0DC
	.4byte fn_800BC21C
	.4byte fn_800BB5F0
	.4byte fn_800B0618
	.4byte fn_800B0610
	.4byte fn_800BEC30
	.4byte fn_800BBABC
	.4byte fn_800BBA9C
	.4byte fn_800B0608
	.4byte fn_800BB738
	.4byte fn_800B0600
	.4byte fn_800BB740
	.4byte fn_800BB748
	.4byte fn_800BB7B0
	.4byte fn_800BB914
	.4byte fn_8003E3C8
	.4byte fn_800B05F8
	.4byte fn_800BE7AC
	.4byte fn_800BEB98
	.4byte fn_800B05EC
	.4byte fn_800BC364
	.4byte fn_800BC2E0
	.4byte fn_800BEA40
	.4byte fn_800BE0DC
	.4byte fn_80092CA8
	.4byte fn_80093924
	.4byte fn_800BE264
	.4byte fn_800BE278
	.4byte fn_800BE2A0
	.4byte fn_800BE2D8
	.4byte fn_800BE2EC
	.4byte fn_800BE300
	.4byte fn_800BE314
	.4byte fn_800BE328
	.4byte fn_800B4B04
	.4byte fn_800BE474
	.4byte fn_800BE504
	.4byte fn_800BE57C
	.4byte fn_800BE6B4
	.4byte fn_800BE734
	.4byte fn_800BE5F4
	.4byte fn_800BE65C
	.4byte fn_800BE858

.obj cf_CfObjectObj_hierarchy, global
	.4byte __RTTI__cf_CObjectState
	.4byte 0x00000000
	.4byte __RTTI__cf_CObjectParam
	.4byte 0x00000000
	.4byte __RTTI__cf_CfObject
	.4byte 0x00000000
	.4byte __RTTI__cf_CfObjectModel
	.4byte 0x00000000
	.4byte __RTTI__cf_CfObjectMove
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000

# 0x80664610 - 0x80664620
.section .sdata, "wa"
.balign 8

.obj __RTTI__cf_CfObjectTbox, global
	.4byte cf_CfObjectTbox_typestr
	.4byte cf_CfObjectTbox_hierarchy

.obj __RTTI__cf_CfObjectObj, global
	.4byte cf_CfObjectObj_typestr
	.4byte cf_CfObjectObj_hierarchy
