...
#   each C(++) file.
#   Repurposed to ensure files end in a newline.
#   Axiot / Intns - 2021
#   Modified by EpochFlame - 2023
...
def format_folders(folder_to_iter):
    for dirpath, dirname, files in os.walk(folder_to_iter):
        for name in files:
            if name.lower().endswith(".h") or name.lower().endswith(".cpp") or name.lower().endswith(".c") or name.lower().endswith(".s"):
