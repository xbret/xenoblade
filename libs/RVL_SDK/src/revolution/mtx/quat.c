...
void PSQUATMultiply(const register Quaternion* quat1,
  ...
  register float vv1, vv2, vv3, vv4;
  register float vv5, vv6, vv7, vv8;
  register float vv9, vvA, vvB, vvC;
  asm
  {
    psq_l vv1, 0(quat1), 0, 0;
...
void PSQUATNormalize(const register Quaternion* src,
  ...
  register float vv7, vv8;
  register float vv9 = 0.00001f;
  register float vvA = 0.5F;
  register float vvB = 3.0F;
  asm
  {
    psq_l    vv1, 0(src), 0, 0;
    ps_mul   vv3, vv1, vv1;
...
void C_QUATMtx(Quaternion* r, const Mtx m) {
  ...
  if (vv0 > 0.0f) {
    ...
    j = idx[i];
    k = idx[j];
    vv1 = (float)sqrtf((m[i][i] - (m[j][j] + m[k][k])) + 1.0f);
    vec[i] = vv1 * 0.5f;
    if (vv1 != 0.0f)
      vv1 = 0.5f / vv1;
    r->w = (m[k][j] - m[j][k]) * vv1;
