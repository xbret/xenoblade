...
static void TCIntrruptHandler(s32 intr, OSContext* ctx) {
    EXIData* exi;
    EXIChannel chan;
    EXICallback callback;
    OSContext temp;
...
static void EXTIntrruptHandler(s32 intr, OSContext* ctx) {
    EXIChannel chan;
    EXIData* exi;
    EXICallback callback;
    OSContext temp;
...
s32 EXIGetID(EXIChannel chan, u32 dev, u32* out) {
    ...
    if (chan < EXI_CHAN_2 && dev == 0) {
        ...
        if (exi->WORD_0x20 == OS_EXI_LAST_INSERT[chan]) {
            *out = exi->id;
    ...
    if (chan < EXI_CHAN_2 && dev == 0) {
        ...
        enabled = OSDisableInterrupts();
        ret |= val != OS_EXI_LAST_INSERT[chan];
        if (ret == 0) {
            exi->id = *out;
            exi->WORD_0x20 = val;
        }
