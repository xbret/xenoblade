...
#else
static void hidd_sec_check_complete(BD_ADDR bd_addr, tBT_TRANSPORT transport, void *p_ref_data, UINT8 res)
#endif
{
    ...
    {
        HIDD_TRACE_WARNING("%s: connection rejected by security", __FUNCTION__);
...
#else
void hidd_sec_check_complete_orig (BD_ADDR bd_addr, tBT_TRANSPORT transport, void *p_ref_data, UINT8 res)
#endif
{
    tHID_DEV_DEV_CTB *p_dev = (tHID_DEV_DEV_CTB *) p_ref_data;
    UINT32 reason;
    ...
    {
        HIDD_TRACE_WARNING("%s: invalid state (%02x)", __FUNCTION__, p_dev->conn.conn_state);
        return;
    }

    if (res == BTM_SUCCESS)
    {
        HIDD_TRACE_EVENT("%s: security ok", __FUNCTION__);
        p_dev->conn.disc_reason = HID_SUCCESS;
    ...
    {
        HIDD_TRACE_WARNING("%s: security check failed (%02x)", __FUNCTION__, res);
        p_dev->conn.disc_reason = HID_ERR_AUTH_FAILED;
...
static void hidd_l2cif_connect_ind(BD_ADDR  bd_addr, UINT16 cid, UINT16 psm, UINT8 id)
{
    ...
    HIDD_TRACE_EVENT("%s: psm=%04x cid=%04x id=%02x", __FUNCTION__, psm, cid, id);

    p_dev  = &hd_cb.device;

    if (!hd_cb.allow_incoming)
    {
        HIDD_TRACE_WARNING("%s: incoming connections not allowed, rejecting", __FUNCTION__);
    ...
    {
        HIDD_TRACE_WARNING("%s: incoming connections from different device, rejecting",
            __FUNCTION__);
        L2CA_ConnectRsp(bd_addr, id, cid, L2CAP_CONN_NO_RESOURCES, 0);
        return;
    }
    ...
    switch (psm) {
        ...
        default:
            accept = FALSE;
            HIDD_TRACE_ERROR("%s: received invalid PSM, rejecting", __FUNCTION__);
            break;
    }
...
static void hidd_l2cif_connect_cfm(UINT16 cid, UINT16 result)
{
    ...
    HIDD_TRACE_EVENT("%s: cid=%04x result=%d", __FUNCTION__, cid, result);

    if (p_hcon->ctrl_cid != cid && p_hcon->intr_cid != cid)
    {
        HIDD_TRACE_WARNING("%s: unknown cid", __FUNCTION__);
        return;
    }
    ...
    {
        HIDD_TRACE_WARNING("%s: unexpected", __FUNCTION__);
        return;
    }

    if (result != L2CAP_CONN_OK)
    {
        HIDD_TRACE_WARNING("%s: connection failed, now disconnect", __FUNCTION__);
...
static void hidd_l2cif_config_cfm(UINT16 cid, tL2CAP_CFG_INFO *p_cfg)
{
    ...
    HIDD_TRACE_EVENT("%s: cid=%04x pcfg->result=%d", __FUNCTION__, cid, p_cfg->result);
        ...
        L2CAP_CFG_UNACCEPTABLE_PARAMS && p_cfg->qos_present)
    {
        ...
        HIDD_TRACE_WARNING("%s: config failed, retry", __FUNCTION__);
        ...
        L2CAP_CFG_UNKNOWN_OPTIONS)
    {
        ...
        HIDD_TRACE_WARNING("%s: config failed, retry without QoS", __FUNCTION__);
    ...
    {
        HIDD_TRACE_WARNING("%s: config failed, disconnecting", __FUNCTION__);
...
static void hidd_l2cif_disconnect_ind(UINT16 cid, BOOLEAN ack_needed)
{
    ...
    HIDD_TRACE_EVENT("%s: cid=%04x ack_needed=%d", __FUNCTION__, cid, ack_needed);
...
static void hidd_l2cif_cong_ind(UINT16 cid, BOOLEAN congested)
{
    ...
    HIDD_TRACE_EVENT("%s: cid=%04x congested=%d", __FUNCTION__, cid, congested);
...
static void hidd_l2cif_data_ind(UINT16 cid, BT_HDR *p_msg)
{
    ...
    case HID_TRANS_DATA:
    default:
        HIDD_TRACE_WARNING("%s: got unsupported msg (%d)", __FUNCTION__, msg_type);
...
tHID_STATUS hidd_conn_initiate(void)
{
    ...
    if (!p_dev->in_use)
    {
        HIDD_TRACE_WARNING("%s: no virtual cable established", __FUNCTION__);
        return (HID_ERR_NOT_REGISTERED);
    }

    if (p_dev->conn.conn_state != HID_CONN_STATE_UNUSED)
    {
        HIDD_TRACE_WARNING("%s: connection already in progress", __FUNCTION__);
    ...
    {
        HIDD_TRACE_WARNING("%s: could not start L2CAP connection", __FUNCTION__);
...
tHID_STATUS hidd_conn_disconnect(void)
{
    ...
    if ((p_hcon->ctrl_cid != 0) || (p_hcon->intr_cid != 0))
    {
        ...
        L2CA_SetIdleTimeoutByBdAddr(hd_cb.device.addr, 0);
    ...
    {
        HIDD_TRACE_WARNING("%s: already disconnected", __FUNCTION__);
        p_hcon->conn_state = HID_CONN_STATE_UNUSED;
    }
