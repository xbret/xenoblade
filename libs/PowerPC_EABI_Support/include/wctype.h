#ifndef _WCTYPE_H
#define _WCTYPE_H

#include "types.h"
#include "locale.h"
#include "PowerPC_EABI_Support/MSL_C/MSL_Common/wctype_api.h"

#ifdef __cplusplus
extern "C" {
#endif


static inline int iswalpha(wint_t c) {
    return (c < 0 || c >= 256) ? 0 : (int)(_current_locale.ctype_cmpt_ptr->wctype_map_ptr[c] & __msl_walpha);
}

static inline int iswdigit(wint_t c) {
    return (c < 0 || c >= 256) ? 0 : (int)(_current_locale.ctype_cmpt_ptr->wctype_map_ptr[c] & __msl_wdigit);
}

static inline int iswspace(wint_t c) { 
    return (c < 0 || c >= 256) ? 0 : (int)(_current_locale.ctype_cmpt_ptr->wctype_map_ptr[c] & __msl_wspace);
}

static inline int iswupper(wint_t c) { 
    return (c < 0 || c >= 256) ? 0 : (int)(_current_locale.ctype_cmpt_ptr->wctype_map_ptr[c] & __msl_wupper);
}

static inline int iswxdigit(wint_t c) { 
    return (c < 0 || c >= 256) ? 0 : (int)(_current_locale.ctype_cmpt_ptr->wctype_map_ptr[c] & __msl_wxdigit);
}

static inline wint_t towlower(wint_t c) {
    return (c < 0 || c >= 256) ? c : (int)(&_current_locale)->ctype_cmpt_ptr->wlower_map_ptr[c];
}
static inline wint_t towupper(wint_t c) {
    return (c < 0 || c >= 256) ? c : (int)(&_current_locale)->ctype_cmpt_ptr->wupper_map_ptr[c];
}

#ifdef __cplusplus
}
#endif
#endif
