...
void CWorkThreadSystem::initialize() {
    sAllocHandle = mtl::MemManager::create(mtl::MemManager::getHandleMEM1(),
                                           REGION_SIZE, scRegionName);

    sMemAvailable = true;

    sAllocFlags = new (sAllocHandle) u32[ALLOC_FLAGS_COUNT];

    sWorkThreads = static_cast<CWorkThread**>(mtl::MemManager::allocate_head(
        sAllocHandle, sizeof(CWorkThread*) * MAX_WORK_ID, 4));
