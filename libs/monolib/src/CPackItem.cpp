...
CPackItem::CPackItem(const char* name, UNKWORD r5) : unk4(), pkbFilename() {
    mFileHandle = nullptr;
    mPackHeader = nullptr;
    unk54 = name;
    unk84 = name;
...
void CPackItem::update(){
    if(mLoadState == LOAD_STATE_NOT_LOADED){
        ...
        if(CWorkSystemPack::func_804DDDF4(unk84, &unk7C, &unk80) != 0){
            ...
            mFileHandle = CDeviceFile::openFile1(mtl::MemManager::getRegionIndex2(), unk84, (UNKTYPE*)this, 0, 0);
        }
        ...
        int length = CPathUtil::getFilenameLengthWithoutExtension((const char*)&pkbFilename - 1, pkbFilename.length);
        ...
        if(mIsAhxAdxFile){
            ...
            u32 r4 = ((mPackHeader->mFiles + 1)*2 + 0x11a) & ~3; //i have no idea what this is supposed to be lol
            mAhxAdxDataPtr = mtl::MemManager::func_8043442C(mtl::MemManager::getRegionIndex2(), r4, 4);
            ADXF_LoadPartitionRangeNw(unk64, pkbFilename.string, 0, mAhxAdxDataPtr);
            mLoadState = LOAD_STATE_LOADING_AHX_ADX_FILE;
        }else{
            mLoadState = LOAD_STATE_LOADED;
        }
    }else if(mLoadState == LOAD_STATE_LOADING_AHX_ADX_FILE){
        if(ADXF_GetPtStat(unk64) == 3){
            mLoadState = LOAD_STATE_LOADED;
        }
    }
}
...
int CPackItem::findHashIndex(int startIndex, int endIndex){
    ...
    if(pHashTable[1] == mHashUpperHalf){
        ...
        findHashIndex(startIndex, midIndex);
    }else{
        findHashIndex(midIndex, endIndex);
    }
}
